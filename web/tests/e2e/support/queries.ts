 ...
 * composed carries that list. Its name states either result; a caller reads
 * `aria-checked` from the same control it presses.
 */
export const cardMembership = (card: Locator, copy: Copy): Locator =>
  card.getByRole('switch', {
    exact: true,
    name: new RegExp(
      `^(?:${escapeRegExp(copy('listCard.membership.active'))}|${escapeRegExp(copy('listCard.membership.inactive'))})$`,
    ),
  })

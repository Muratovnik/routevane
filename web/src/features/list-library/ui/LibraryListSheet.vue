...
function parsedDomains(): string[] | null {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (domain === null) {
      domainsError.value = t('serviceCard.new.domains.invalid', {
        line: index + 1,
      })
      return null
    }
  ...
  if (unique.length > 64) {
    domainsError.value = t('serviceCard.new.domains.limit')
    return null
  }
  domainsError.value = ''
  return unique
...
function submit(): void {
  ...
  titleError.value = nextTitle === '' ? t('serviceCard.title.invalid') : ''
  ...
  >
    <form class="library-list-sheet" @submit.prevent="submit">
      ...
      <template v-else>
        ...
        />
        <p v-else class="library-list-sheet__note">

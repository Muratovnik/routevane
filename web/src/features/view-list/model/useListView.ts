...
export type ListState = 'loading' | 'ready' | 'missing' | 'failed'
...
/**
 * One stored list and the formats it publishes in. The server names its facts;
 ...
 */
export function useListView(listID: () => string) {
  const publishedRoute = usePublishedRoute()
  const preferences = useSurfacePreferences()
  const { locale } = useLocale()

  const state = ref<ListState>('loading')
  const list = ref<RouteList | null>(null)
  ...
  // An archived list is fully readable and fully downloadable; what it no
  ...
  const archived = computed(() => list.value !== null && isArchived(list.value))
  ...
  const fresh = computed(() =>
    selectedOutputID.value === ''
      ? null
      : publishedRoute.forOutput(selectedOutputID.value),
  )
  ...
  // What the list publishes right now, as the server resolved it. The screen
  // never resolves references itself: a stale catalog copy would disagree with
  // the build.
  const serviceTitles = computed<string[]>(() =>
    resolved.value.map(serviceTitle),
  )

  // A target already bound to this list is not offered again, and a target
  ...
  const rulesByService = computed<{ id: string; count: number }[]>(() => {
    const counts = new Map<string, number>()
    for (const rule of diagnostics.value) {
      if (rule.excluded) continue
      counts.set(rule.serviceID, (counts.get(rule.serviceID) ?? 0) + 1)
    }
  ...
  async function initialize(): Promise<void> {
    ...
    if (detail.status === 'rejected') {
      // A list the server has never heard of (404) is a different fact from a
      // list the server cannot answer for right now: only the first is
      // "missing". Everything else — 5xx, a network failure, a contract the
      // client cannot decode — leaves the list possibly still there, so the
  ...
  /**
   * publish refreshes the list once and rebuilds the outputs named. A failure
  ...
  async function publish(ids: string[]): Promise<boolean> {
    ...
    for (const id of ids) {
      try {
        ...
        publishedRoute.publish({
          ...built,
          stale: false,
          subscriptionURL:
            built.subscriptionURL ||
            publishedRoute.forOutput(id)?.subscriptionURL ||
            '',
        })
      } catch {
        complete = false
   ...
   * saved list and its files never quietly disagree.
   */
  async function save(
    name: string,
    composition: ListComposition,
  ): Promise<boolean> {
    if (busy.value || list.value === null) return false
    work.value = 'saving'
    rebuildFailed.value = false
    try {
      await updateList(listID(), name, composition)
    } catch {
      work.value = 'failed'
      return false
    }
  ...
  /**
   * setSchedule records this list's own refresh rule. It does not refresh: the
  ...
  async function setSchedule(interval: RefreshInterval): Promise<boolean> {
    ...
    try {
      schedule.value = await saveListRefreshInterval(listID(), interval)
      return true
    } catch {
      return false
    }
  ...
  /**
   * setArchived takes the list off the shelf or puts it back. Neither direction
   * refreshes or rebuilds: archiving must not touch the file subscribers are
   * receiving, and restoring returns the list exactly as it left.
   */
  async function setArchived(next: boolean): Promise<boolean> {
    if (busy.value || list.value === null) return false
    work.value = 'archiving'
    try {
      await (next ? archiveList(listID()) : restoreList(listID()))
      await initialize()
      work.value = 'idle'
      return true
    } catch {
  ...
  /**
   * bind adds a format to this list and publishes it at once, because a format
  ...
  async function bind(targetID: string): Promise<boolean> {
    ...
    rebuildFailed.value = false
    try {
      const created = await addOutput(listID(), targetID)
      await refreshList(listID())
  ...
  async function download(formatID: string): Promise<boolean> {
    ...
    try {
      await downloadListExport(listID(), formatID)
      return true
    } catch {
   ...
   * that fact. Editing a route writes nothing global (ADR 0029), so the change

...
describe('ListPicker', () => {
  ...
  it('bulk selection applies to filtered rows and preserves hidden and category selections', async () => {
    ...
    await search.setValue('no matching list')
  ...
  it('bulk selects the union of category filters once and preserves hidden selections', async () => {
    const wrapper = mountPicker({
      modelValue: { ...emptyComposition, lists: ['steam'] },
    })
    wrapper
  ...
  it('keeps row order and search state while selection changes', async () => {
    const wrapper = mountPicker({
      modelValue: { ...emptyComposition, lists: ['discord'] },
    })

    await wrapper.setProps({
      modelValue: { ...emptyComposition, lists: ['steam', 'discord'] },
    })
    expect(
  ...
  it('opens on the contents table with origins and no row controls', async () => {
    ...
    expect(card.querySelector('.list-card__rows')?.textContent).not.toContain(
      'from source',
    )
    expect(card.textContent).toContain('3 entries on')
    expect(card.querySelectorAll('.list-card__rows li')).toHaveLength(5)
    expect(
      card.querySelectorAll('.list-card__rows input[type="checkbox"]'),
    ).toHaveLength(0)
    expect(wrapper.emitted('update:modelValue')).toBeUndefined()
    wrapper.unmount()
  })
  ...
  it('filters the contents by value and by origin', async () => {
    ...
    await flushPromises()
    expect(card.querySelectorAll('.list-card__rows li')).toHaveLength(2)
    expect(card.textContent).toContain('198.51.100.7')
    ...
    field!.dispatchEvent(new Event('input'))
    await flushPromises()
    expect(card.querySelectorAll('.list-card__rows li')).toHaveLength(1)

    field!.value = 'nothing-here'
    field!.dispatchEvent(new Event('input'))
    await flushPromises()
    expect(card.querySelectorAll('.list-card__rows li')).toHaveLength(0)
  ...
  it('states membership without naming an unsaved draft', async () => {
    ...
    const membership = card.querySelector('.list-card__membership')
  ...
  it('names the profile once it is stored', async () => {
    ...
    const wrapper = mountPicker({ profileName: 'Chat and video' })

    await wrapper.findAll('.picker__open')[0]?.trigger('click')
    await flushPromises()

    expect(
      openDialog().querySelector('.list-card__membership')?.textContent,
  ...
  it('names an unnamed draft as this profile', async () => {
    ...
    const wrapper = mountPicker({ profileName: '   ' })
    ...
    expect(
      card.querySelector('.list-card__membership')?.textContent?.trim(),

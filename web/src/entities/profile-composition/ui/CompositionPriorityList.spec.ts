...
describe('CompositionPriorityList', () => {
  ...
  it('shows the composition without visible ordinals and supports keyboard reordering', async () => {
    ...
    expect(
      wrapper.findAll('.priority-profile__item').map((row) => row.text()),
    ).toEqual(['Alpha', 'Beta'])
    expect(wrapper.find('.priority-profile__position').exists()).toBe(false)
    expect(
      wrapper.get('.priority-profile__handle').attributes('aria-label'),
    ).toContain('position 1')
    await wrapper.get('.priority-profile__handle').trigger('keydown', {
      key: 'ArrowDown',
    })
  ...
  it('disables its drag handles with the owning operation', () => {
    ...
    expect(
      wrapper.get<HTMLButtonElement>('.priority-profile__handle').element
        .disabled,
    ).toBe(true)
    wrapper.unmount()
  })

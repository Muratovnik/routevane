...
func TestPlanningSnapshotReportsSourceHealthForGraceDecisions(t *testing.T) {
	...
	profileJSON, err := json.Marshal(domain.RawJSONTargetDefinition())
	if err != nil {
		t.Fatal(err)
	}
	...
	sighting := domain.Sighting{ListID: list, ComponentID: "web", Resource: resource, SourceID: "dns-main", SourceClass: domain.SourceObserved, SourceRevision: dnsRevision, FirstSeen: t0, LastSeen: t0, ValidUntil: t0.Add(time.Hour), ObservationCount: 1, Validity: domain.ValidityValid}
	if err := store.ApplySuccess(context.Background(), SuccessCycle{ListID: list, SourceID: "dns-main", SourceRevision: dnsRevision, StartedAt: t0, CompletedAt: t0, Sightings: []domain.Sighting{sighting}, Format: &format}); err != nil {
		t.Fatal(err)
	}
	if err := store.ApplySuccess(context.Background(), SuccessCycle{ListID: list, SourceID: "official-feed", SourceRevision: feedRevision, StartedAt: t0, CompletedAt: t0}); err != nil {
		t.Fatal(err)
	}
	...
	if err := store.RecordFailure(context.Background(), FailureCycle{ListID: list, SourceID: "official-feed", SourceRevision: feedRevision, StartedAt: failedAt, CompletedAt: failedAt, ErrorCode: application.SourceErrorCode(domain.SourceHTTP)}); err != nil {
		t.Fatal(err)
	}
	snapshot, err = store.ReadPlanningSnapshot(context.Background(), list, active, "raw-v1", failedAt)
	if err != nil {
		t.Fatal(err)
	}
	health = healthByID(t, snapshot.SourceHealth)
	...
	snapshot, err = store.ReadPlanningSnapshot(context.Background(), list, active, "raw-v1", afterWindow)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := store.ApplySuccess(context.Background(), SuccessCycle{ListID: list, SourceID: "official-feed", SourceRevision: feedRevision, StartedAt: recoveredAt, CompletedAt: recoveredAt}); err != nil {
		t.Fatal(err)
	}
	snapshot, err = store.ReadPlanningSnapshot(context.Background(), list, active, "raw-v1", recoveredAt)
	if err != nil {
		t.Fatal(err)
	}
	health = healthByID(t, snapshot.SourceHealth)
...
func TestPlanningSnapshotSeparatesHealthPerSourceRevision(t *testing.T) {
	...
	format := FormatRecord{FormatKey: "raw-v1", ListID: "example", TargetID: "raw-json", RendererID: "raw-json", CatalogRevision: strings.Repeat("a", 64), ConfigJSON: profileJSON, UpdatedAt: t0}
	if err := store.ApplySuccess(context.Background(), SuccessCycle{ListID: "example", SourceID: "official-feed", SourceRevision: "old", StartedAt: t0, CompletedAt: t0, Format: &format}); err != nil {
		t.Fatal(err)
	}
	failedAt := t0.Add(time.Hour)
	if err := store.RecordFailure(context.Background(), FailureCycle{ListID: "example", SourceID: "official-feed", SourceRevision: "new", StartedAt: failedAt, CompletedAt: failedAt, ErrorCode: "http_feed_failed"}); err != nil {
		t.Fatal(err)
	}

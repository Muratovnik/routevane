...
func (s *Store) CreateOutput(ctx context.Context, create application.NewOutput) error {
	...
	if !validID(o.ID) || !validID(o.ListID) || o.TargetID == "" || o.ProfileKey == "" || o.RendererID == "" || o.RendererVersion == "" || o.TargetRevision == "" || o.CreatedAt.IsZero() || (hasToken && (!validID(create.TokenID) || create.TokenHash == ([32]byte{}))) {
...
func (s *Store) OutputsByList(ctx context.Context, listID string) ([]application.Output, error) {
	if !validID(listID) {
...
func (s *Store) Outputs(ctx context.Context) ([]application.Output, error) {
	ctx, cancel := bounded(ctx)
	defer cancel()
	return s.scanOutputs(ctx, outputSelect+` ORDER BY created_at_ns DESC, id ASC LIMIT ?`, listLimit*4)
}
